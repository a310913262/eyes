...
/**
 * 
 * LDAP attribute mapper class
 * 
 */
public class AdUserAttributeMapper implements AttributesMapper {

	public AdUser mapFromAttributes(Attributes attributes) throws NamingException {
		AdUser bean = null;
		if (AdUser.class.isAnnotationPresent(Entry.class)) {// 是否加@Entry
			try {
				bean = AdUser.class.newInstance();
			} catch (Exception e) {
